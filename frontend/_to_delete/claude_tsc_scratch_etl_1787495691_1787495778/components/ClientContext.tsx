...
interface ClientContextType {
  ...
  authReady: boolean;
  // Added: lets a consumer (e.g. SwarmLogStreamer's "Retry Authentication"
  // button) force a fresh dev-login attempt after a failure, without
  // duplicating the login logic itself or standing up a second,
  // independently-managed token store. Bumping authAttempt re-runs the
  // same effect that runs on mount / clientId change.
  retryLogin: () => void;
}
...
export function ClientProvider({ children }: { children: ReactNode }) {
  ...
  }, [clientId, authAttempt]);

  const retryLogin = () => setAuthAttempt((n) => n + 1);

  return (
    <ClientContext.Provider value={{ clientId, setClientId, authToken, authReady, retryLogin }}>
      {children}
