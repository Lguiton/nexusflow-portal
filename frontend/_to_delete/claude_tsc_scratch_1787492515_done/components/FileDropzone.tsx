...
// Safe environment variable accessor to eliminate process type errors in browser context
...
export default function SwarmLogStreamer({ sessionId = "demo_session" }: { sessionId?: string }) {
  ...
  useEffect(() => {
    ...
    const connectWebSocket = () => {
      ...
      socket.onopen = () => {
        if (isCurrent) {
          setConnected(true);
          console.log("[Swarm Telemetry] Secure WebSocket connected.");
        }
      };
      ...
      socket.onmessage = (event: MessageEvent) => {
        ...
        try {
          ...
          const rawData: JsonRecord = parsedData && typeof parsedData === "object" ? parsedData as JsonRecord : {};
          const status = rawData.status;
          const payload = rawData.payload;
          const data: LogStep = {
            agent: typeof rawData.agent === "string" ? rawData.agent.slice(0, 50) : "UnknownAgent",
            status: status === "PROCESSING" || status === "COMPLETE" || status === "ERROR" ? status : "COMPLETE",
            payload: payload && typeof payload === "object" ? payload as JsonRecord : {},
          ...
          console.warn("[Swarm Telemetry] Failed to parse incoming message:", err);
        }
      };
  ...
  const handleDevAuth = async () => {
    try {
      const baseUrl = getEnv("NEXT_PUBLIC_API_URL", "http://127.0.0.1:8000");
      ...
      console.error("[Swarm Telemetry] Failed to authenticate dev token:", err);
    }
  };
  ...
  const sendQuery = (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      console.error("[Swarm Telemetry] Failed to send query:", err);
    } finally {
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-2xl text-slate-100 flex flex-col h-[500px]">
      <div className="flex items-center justify-between border-b border-slate-800 pb-3 mb-4">
        ...
        >
          {authError ? "Unauthorized" : connected && verifiedTenant ? `Verified Tenant: ${verifiedTenant}` : "Authenticating..."}
        </span>
      </div>
      ...
      {!authError && !connected && (
        <div className="flex items-center gap-2 bg-amber-950/30 border border-amber-900/50 rounded-lg p-3 text-amber-400 text-xs mb-3">
          <WifiOff className="w-4 h-4 animate-pulse" />
          Attempting to re-establish secure telemetry link...
        </div>
      )}
