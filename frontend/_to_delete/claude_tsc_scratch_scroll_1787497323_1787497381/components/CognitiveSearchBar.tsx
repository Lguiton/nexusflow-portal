...
export default function CognitiveSearchBar({ onQueryResult }: CognitiveSearchBarProps) {
  ...
  // the request body. /api/search requires
  // Depends(verify_jwt_and_get_client_id) (Authorization: Bearer only --
  // it has never read a body field), and SearchRequest itself has no
  // client_id field to receive that value even if it were read. Net
  // effect: every query through this, the dashboard's main "Universal
  // Cognitive Search" bar, has always 401'd, and the raw backend error
  // string ("Missing or malformed Authorization header.") rendered
  // straight into the UI via the `data.detail` fallback below -- the same
  // bug class as the Live Swarm Telemetry / ETLDropzone findings, just in
  // the flagship interactive feature this time.
  const { authToken, authReady, retryLogin } = useClientId();

  const handleSearch = async (e: React.FormEvent) => {
    e.preventDefault();
    const trimmedQuery = query.trim();

    if (!trimmedQuery || !authToken) return;
    ...
    try {
      const res = await fetch(`${apiUrl}/api/search`, {
        ...
        // client_id is no longer sent here -- the backend has only ever
        // derived tenant identity from the verified JWT above, never from
        // this body, and SearchRequest has no field to receive it.
      ...
      if (!res.ok || data.status === 'ERROR') {
        // 401/403 here means the auth layer itself rejected the request --
        // infrastructure detail, not something about the query -- so
        // translate rather than show the raw backend string, and route
        // through the same Retry Authentication recovery every other
        // widget uses. Other failures (e.g. a downstream agent error) are
        // still shown as returned; that's genuine, useful feedback.
      ...
      if (err.name === 'AbortError') {
        ...
        setError('Your session needs to be re-authenticated before searching.');
      } else {
  ...
  return (
    <div className="w-full bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-md text-slate-100 my-6">
      ...
      {authBlocked ? (
        <div className="bg-red-950/40 border border-red-900/60 rounded-xl p-4 flex flex-col sm:flex-row items-start sm:items-center gap-3 text-red-300 text-sm">
          <div className="flex items-center gap-2 flex-1">
            <ShieldAlert className="w-5 h-5 flex-shrink-0 text-red-400" />
            <span>Unable to authenticate -- search is unavailable until this is resolved.</span>
          </div>
          <button
            onClick={() => retryLogin()}
            className="bg-indigo-600 hover:bg-indigo-500 text-white px-3 py-2 rounded text-xs font-semibold transition-colors whitespace-nowrap"
          >
        ...
        <form onSubmit={handleSearch} className="relative flex items-center">
          ...
          <button
            type="submit"
            disabled={loading || !query.trim() || !authReady}
